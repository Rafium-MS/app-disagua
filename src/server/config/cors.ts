...
export const buildCorsOptions = ({ env = process.env }: BuildCorsOptionsParams = {}): CorsOptions => {
  const configuredOrigins = parseOrigins(env.CORS_ALLOWED_ORIGINS)
  const fallbackOrigins = configuredOrigins.length > 0 || env.NODE_ENV === 'production'
    ? []
    : ['http://localhost:5173']
